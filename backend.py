...
async def chat_send(req: ChatRequest, token: dict = Depends(verify_token), db: sqlite3.Connection = Depends(get_db)):
    ...
    # ── Gemini API call ─────────────────────────────────────────────────────
    import httpx
    api_key = os.getenv("GEMINI_API_KEY", "AIzaSyBOw0ojqDUOUPnY_qBFNSdjq-wOwsfTFOU")
    ...
    if resp.status_code == 200:
        ...
        ai_response = f"API xətası: {resp.status_code} — {resp.text[:200]}"
...
def on_startup():
    ...
    print(f"  DB: {DB_PATH}")
    print(f"  Gemini API Key: {'✓ SET' if os.getenv('GEMINI_API_KEY') else '✓ HARDCODED (change in prod!)'}")
    print("="*50 + "\n")
